...
@torch.no_grad()
def test_process_feature_extract(model, criterion, dataloader, device, num_bags=5,
                                 aggregate="max"):
    assert aggregate == "mean" or aggregate == "max", "aggregate must be mean or max"
    cpu_device = torch.device("cpu")
    model.eval()
    ...
    for i in range(num_bags):
        ...
        try:
            ...
            all_features.append(np.vstack(model.module.extracted_features))
        except RuntimeError:
            print("RuntimeError")
            print(slide_names)
    ...
    df = pd.DataFrame(all_features_)
    df['slide_name'] = all_slide_names * 10
    df['label'] = labels * 10
    # df_new = df.groupby('slide_name').max()
